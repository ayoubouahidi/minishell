...
void				process_word_token(t_token *token, char ***args,
						bool *in_red);
t_command			*init_command(void);
void				ft_lstadd_back_red(t_redirections **lst,
						t_redirections *new);
t_redirections		*creat_red(char *file, t_TYPE_TOKEN type);
bool				heredoc_check_append(t_token *token,
						char ***heredoc_delimiters, int *heredoc_count,
						bool **heredoc_quoted);
...
bool				is_token_delimiter(char c, int flag_single,
						int flag_double);
...
void				handle_child_process(char *filename, t_command *cmd,
						t_env *env);
